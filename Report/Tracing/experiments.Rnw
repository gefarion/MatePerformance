...
tracing$VM[tracing$Var == FALSE & tracing$VM == "MATEpe-Ent"] <- "MATEpe-baseline"
...
tracingPlot <- boxplot(list(tracingMT, tracingPE), TRUE, "Run Time", list('MATEmt-baseline', 'MATEpe-baseline'), fill = TRUE)
print(tracingPlot)
@
}%
...
tracingSummary <- rbind(tracingSummaryPE, tracingSummaryMT)[,c(2,1,7,8,3:6)]
print(
