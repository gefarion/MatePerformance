...
<<perf-overview-mop, fig.width=5, fig.height=2, fig.align='center', fig.cap='Overhead factor for redefining VM operations at the language-level. The baseline for each bench, is the execution of the same benchmark in the same VM but without redefining the corresponding VM operation.', fig.pos = 'H'>>=
print(boxplot(list(mopNormalized), TRUE, "Overhead Factor", list(), fill = TRUE))
@
}%
