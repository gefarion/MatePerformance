...
micro <- droplevels(subset(steady, (Suite == "classic-micro-steady" | Benchmark %in% microBenchmarks) & Benchmark != "Loop"))
...
microMTSummary <- summarizedPerBenchmark(microMT, sommtSteady , "SOMmt", c(0,10000))
microPESummary <- summarizedPerBenchmark(microPE, steadyPE, "SOMpe", c(0,10000))
...
microMTSummaryWithoutRecursive <- summarizedPerBenchmark(microMTWithoutRecursive, sommtSteady, "SOMmt", c(0,10000))
microPESummaryWithoutRecursive <- summarizedPerBenchmark(microPEWithoutRecursive, sompeSteady, "SOMpe", c(0,10000))

macroMT <- droplevels(subset(macro, VM %in% mtVMs))
macroPE <- droplevels(subset(macro, VM %in% peVMs))
macroMTSummary <- summarizedPerBenchmark(macroMT, sommtSteady, "SOMmt", c(0,10000))
macroPESummary <- summarizedPerBenchmark(macroPE, sompeSteady, "SOMpe", c(0,10000))
@
...
<<perf-overview-micro, fig.cap='Overhead of TruffleMate normalized to TruffleSOM, clustered by micro and macro benchmarks, without (above) and with (below) optimizations', out.width = "0.8\\textwidth", fig.align='center', fig.width=8, fig.height=4>>=
boxplotMateToSom(list(microMTWithoutRecursive, microPEWithoutRecursive), list('SOMmt', 'SOMpe'), yTitle = "Overhead Factor", xTitle="Benchmark", title="Mate Overheads to SOM for Non-Reflective Micro Benchmarks")
@
}
...
<<perf-overview-macro, fig.cap='Overhead of TruffleMate normalized to TruffleSOM, clustered by micro and macro benchmarks, without (above) and with (below) optimizations', out.width = "0.8\\textwidth", fig.align='center', fig.width=8, fig.height=4>>=
...
if (includeNaive){
  overallMicro <- arrange(summarizeOverall(rbind(summaryDataMicro, naiveMicroWithoutRecursiveSummary), ~ VM), OF2)
  overallMacro <- arrange(summarizeOverall(rbind(summaryDataMacro, naiveMacroSummary), ~ VM), OF2)
  ...
  overallMicro <- arrange(summarizeOverall(rbind(summaryDataMicro), ~ VM), OF2)
  overallMacro <- arrange(summarizeOverall(rbind(summaryDataMacro), ~ VM), OF2)
