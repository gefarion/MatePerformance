...
<<perf-overview-readonly, out.width = "0.7\\textwidth", fig.cap='Overhead factors of implementing read-only reference using Handles with Mate (MOP) and using delegation proxies (Proxies).', fig.align='center', fig.pos="H">>=
