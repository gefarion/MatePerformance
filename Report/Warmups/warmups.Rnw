...
allOperationsMop <- droplevels(subset(allOperationsMop, Benchmark != "Mega" & Benchmark != "Mega2" & Benchmark != "Mono" & Benchmark != "Mono2"))
...
rawRO$VM[rawRO$Benchmark == "SumKeys" & rawRO$VM == "MATEpe-Ent"] <- "MATEpe-baseline"
...
rawTracing$VM[rawTracing$Var == FALSE & rawTracing$VM == "MATEpe-Ent"] <- "MATEpe-baseline"
...
plotsforPaper <- c("DeltaBlue", "Havlak", "Json", "Readonly", "Richards", "NBody","Mandelbrot", "Bounce", "CD", "FieldRead", "Activation", "All", "CDT", "LocalWrite", "JsonT")
...
for (b in levels(warmups$Benchmark)) {
  ...
  ci <- ciForVM(summaries, b, c("MATEpe-Ent", "MATEmt"))
...
for (i in iterations){
  # AWF + Inherent
  iterRange <- c(0,i)
  summarizedPE <- overheadFactors(subset(rawAWF, VM == "MATEpe-Ent"), subset(rawAWF, VM == "SOMpe-Ent"), iterRange)
  ...
  # Individual
  summaryIndividualPE <- overheadFactors(subset(allOperationsMop, VM == "MATEpe-Ent" & Benchmark != "FieldWrite"), subset(allOperationsMop, VM == "MATEpe-baseline" & Benchmark != "FieldWrite"), iterRange)
  
  summaryIndividualMT <- overheadFactors(subset(allOperationsMop, VM == "MATEmt" & Benchmark != "FieldWrite"), subset(allOperationsMop, VM == "MATEmt-baseline" & Benchmark != "FieldWrite"), iterRange)  
  ...
  # Readonly
  summaryROPE <- overheadFactors(subset(rawRO, VM == "MATEpe-Ent"), subset(rawRO, VM == "MATEpe-baseline"), iterRange)
  ...
  # Tracing 
  summaryTracingPE <- overheadFactors(subset(rawTracing, VM == "MATEpe-Ent"), subset(rawTracing, VM == "MATEpe-baseline"), iterRange)
...
columnNames <- c("Iterations", "VM", "Mean","CI-95%","Sd.", "Min", "Max", "Median")
